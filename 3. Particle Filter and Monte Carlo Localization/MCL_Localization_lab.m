...
% Legi Number: 19-945-567
% Name: Paccagnella, Alessia
...
measurement = (door1 + door2 + door3)/sum(door1 + door2 + door3);
...
% TODO: Change this value for the correct one
predict_particles = priorbelief_particles + uk + simpar.odometry_stdev.*randn(1, simpar.numberOfParticles); %because when you have circular halls you need the mod to go back
...
particle_weights = (door1 + door2 + door3)/sum(door1 + door2 + door3); %isn't it equal to the domain?
...
% hai il vettore di pesi. scrivi il vettore delle somme parziali. generi un
% numero random e lo inseriresti dove? il posto in cui lo inserisci è il
% coso che prendi
% TODO: Change this value for the correct one

particle_weights = particle_weights/ sum(particle_weights);
P = cumsum(particle_weights);
j = 1;
for i = 1 : simpar.numberOfParticles 
    r = rand(1);
    j = find(P >= r, 1);
    updated_belief_particles(i) = predict_particles(j);
end
